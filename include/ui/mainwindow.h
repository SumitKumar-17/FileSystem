...
class MainWindow : public QMainWindow {
  ...
  public:
    ...
    FileSystemCheck *getFsCheck() {
        return fsck.get();
    }
    FileSystemSearch *getSearch() {
        return search.get();
    }
    QuotaManager *getQuotaManager() {
        return quotaManager.get();
    }
    SnapshotManager *getSnapshotManager() {
        return snapshotManager.get();
    }
    FileSystemDetector *getFsDetector() {
        return fsDetector.get();
    }

    // Accessor for current open file
    std::string getCurrentOpenFile() {
        return current_open_file;
    }
    void setCurrentOpenFile(const std::string &path) {
