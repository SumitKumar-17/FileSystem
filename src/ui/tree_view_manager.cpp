...
void TreeViewManager::refreshTreeView()
{
    ...
    // Create root item
    QStandardItem *rootItem = new QStandardItem(QIcon(treeView->style()->standardIcon(QStyle::SP_DirIcon)), "/");
...
void TreeViewManager::buildDirectoryTree(QStandardItem *parentItem, int parent_inode, const std::string &parent_path)
{
    if (!fs) return;
