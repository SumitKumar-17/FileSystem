...
void FileSystemMountDialog::setupUI()
{
    ...
    QPushButton *refreshButton = new QPushButton(QIcon(style()->standardIcon(QStyle::SP_BrowserReload)), "Refresh Filesystem List");
...
void FileSystemMountDialog::populateFilesystemList(const QStringList &availableFilesystems)
{
    ...
    for (const QString &fs : availableFilesystems) {
        if (FileSystemDetector::isExternalPath(fs)) {
            ...
            QListWidgetItem *item = new QListWidgetItem(QIcon(style()->standardIcon(QStyle::SP_DriveHDIcon)), displayName);
    ...
    for (const QString &fs : availableFilesystems) {
        if (!FileSystemDetector::isExternalPath(fs)) {
            ...
            QListWidgetItem *item = new QListWidgetItem(QIcon(style()->standardIcon(QStyle::SP_FileIcon)), displayName);
            item->setData(Qt::UserRole, fs);
