...
MainWindowDialogs::MainWindowDialogs(MainWindow *mainWindow) : mainWindow(mainWindow) {
}
...
void MainWindowDialogs::handleFsCheck() {
    ...
    if (issues.empty()) {
        QMessageBox::information(mainWindow, "Filesystem Check",
                                 "Filesystem check completed successfully. No errors found.");
    } else {
...
void MainWindowDialogs::handleAdvancedSearch() {
    ...
    bool ok;
    QString searchTerm = QInputDialog::getText(mainWindow, "Advanced Search",
                                               "Enter search term:", QLineEdit::Normal, "", &ok);
...
void MainWindowDialogs::handleSnapshots() {
    ...
    QObject::connect(createButton, &QPushButton::clicked, [this, snapshotList]() {
        ...
        QString name = QInputDialog::getText(
            mainWindow, "Create Snapshot", "Enter snapshot name:", QLineEdit::Normal,
            "snapshot_" + QDateTime::currentDateTime().toString("yyyy-MM-dd_hh-mm-ss"), &ok);
    ...
    QObject::connect(restoreButton, &QPushButton::clicked, [this, snapshotList]() {
        ...
        QMessageBox::StandardButton reply =
            QMessageBox::question(mainWindow, "Confirm Restore",
                                  "Are you sure you want to restore snapshot " + item->text() +
    ...
    QObject::connect(deleteButton, &QPushButton::clicked, [this, snapshotList]() {
        ...
        QMessageBox::StandardButton reply =
            QMessageBox::question(mainWindow, "Confirm Delete",
        ...
        if (item) {
            ...
            if (selectedPath.startsWith("UNMOUNTED:")) {
                ...
                QMessageBox::information(mainWindow, "Mount",
                                         "Selected unmounted device: " + devicePath);

                // Ask if user wants to mount this device first
                QMessageBox::StandardButton reply = QMessageBox::question(
                    mainWindow, "Mount Device",
                ...
                if (reply == QMessageBox::Yes) {
                    ...
                    if (mountProcess.waitForFinished(5000)) {
                        if (mountProcess.exitCode() == 0) {
                            ...
                            QMessageBox::information(mainWindow, "Success",
                                                     "Device successfully mounted at " +
                                                         mountPoint);
                            ...
                            if (newFs->mount()) {
                                ...
                                mainWindow->updateStatusBar("Mounted external filesystem: " +
                                                            mountPoint);
                            } else {
                                delete newFs;
                                QMessageBox::warning(
                    ...
                    QProgressDialog progress("Creating temporary filesystem image...", "Abort", 0,
                                             100, mainWindow);
                    ...
                    if (blockdevProcess.waitForFinished(2000)) {
                        QString sizeStr =
                            QString::fromUtf8(blockdevProcess.readAllStandardOutput()).trimmed();
                    ...
                    QProcess ddProcess;
                    ddProcess.start("dd", QStringList() << "if=" + devicePath
                                                        << "of=" + tempFilePath << "bs=1M");
                    ...
                    if (!success) {
                        QMessageBox::critical(mainWindow, "Error",
                                              "Failed to create image of external device");
                        return;
                    }
                    ...
                    if (!newFs->mount()) {
                        QMessageBox::critical(mainWindow, "Error",
                                              "Failed to mount temporary filesystem image");
                        delete newFs;
                        return;
                    }
            ...
            if (FileSystemDetector::isExternalPath(selectedPath)) {
                ...
                QMessageBox::information(mainWindow, "Mount",
                                         "Selected external filesystem: " + realPath);

                // Attempt to create a temporary file from the external filesystem content
                QString tempFilePath = QDir::tempPath() + "/temp_external_fs.fs";

                QProgressDialog progress("Creating temporary filesystem image...", "Abort", 0, 100,
                                         mainWindow);
                progress.setWindowModality(Qt::WindowModal);
                progress.show();
                ...
                findmntProcess.start("findmnt", QStringList()
                                                    << "-n" << "-o" << "SOURCE" << realPath);
                if (findmntProcess.waitForFinished(2000)) {
                    sourceDevice =
                        QString::fromUtf8(findmntProcess.readAllStandardOutput()).trimmed();
                }

                if (sourceDevice.isEmpty()) {
                    QMessageBox::critical(mainWindow, "Error",
                                          "Could not determine source device for mount point: " +
                                              realPath);
                    return;
                }
                ...
                // Use dd to create the image file
                ddProcess.start("dd", QStringList() << "if=" + sourceDevice << "of=" + tempFilePath
                                                    << "bs=1M");
                ...
                QObject::connect(&ddProcess, &QProcess::readyReadStandardError, [&]() {
                    ...
                    if (output.contains("bytes")) {
                        int progressValue =
                            30 + QRandomGenerator::global()->bounded(50); // Simulate progress
                ...
                if (!ddProcess.waitForFinished(30000)) { // 30 seconds timeout
                    QMessageBox::critical(mainWindow, "Error",
                                          "Timeout while creating filesystem image.");
                    return;
                }
                ...
                try {
                    ...
                    if (newFs->mount()) {
                        ...
                        QMessageBox::critical(mainWindow, "Error",
                                              "Failed to mount the temporary filesystem image.");
                    }
                } catch (const std::exception &e) {
                    QMessageBox::critical(mainWindow, "Error",
                                          "Exception occurred while mounting filesystem: " +
                                              QString(e.what()));
                }
                ...
                try {
                    ...
                    if (newFs->mount()) {
                        ...
                        QMessageBox::information(mainWindow, "Mount",
                                                 "Successfully mounted filesystem: " + actualPath);
                    } else {
                        delete newFs; // Delete if mount failed
                        QMessageBox::critical(mainWindow, "Mount",
                                              "Failed to mount filesystem: " + actualPath);
                    }
                } catch (const std::exception &e) {
                    QMessageBox::critical(mainWindow, "Error",
                                          "Exception occurred while mounting filesystem: " +
