...
QString FileSystemDetector::getDisplayNameForPath(const QString &path) {
    if (path.startsWith("UNMOUNTED:")) {
        ...
        processDetails.start("lsblk", QStringList()
                                          << "-n" << "-o" << "VENDOR,MODEL,LABEL,SIZE,TYPE,FSTYPE"
                                          << realPath);
        ...
        if (processDetails.waitForFinished(1000)) {
            ...
            if (!output.isEmpty()) {
                ...
                udevProcess.start("udevadm",
                                  QStringList() << "info" << "--query=all" << "--name=" + realPath);
        ...
        if (process.waitForFinished(1000)) {
            ...
            if (!output.isEmpty()) {
                ...
                if (parts.size() >= 2) {
                    ...
                    if (parts[0].contains("G") || parts[0].contains("M") ||
                        parts[0].contains("K")) {
                        // It's likely a size
