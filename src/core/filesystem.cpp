...
FileSystem::FileSystem(const std::string &name)
    : disk_name(name), current_dir_inode(0), journal(nullptr) {
}
...
void FileSystem::add_dir_entry(int dir_inode_num, const std::string &name, int new_inode_num) {
    if (!is_valid_inode(dir_inode_num) || inodes[dir_inode_num].mode != 2) {
        if (dir_inode_num >= 0) {
            qDebug() << "Warning: Attempted to add entry to invalid directory inode:"
                     << dir_inode_num;
        }
        return;
    }
...
bool FileSystem::mount() {
    ...
    if (is_external) {
        ...
        if (!test_path.good()) {
            std::cerr << "External path doesn't exist or is not accessible: " << disk_name
                      << std::endl;
            return false;
        }
        ...
        inodes[0].creation_time = inodes[0].modification_time = inodes[0].access_time =
            time(nullptr);
...
void FileSystem::mkdir(const std::string &dirname) {
    ...
    memcpy(inode_buffer, &inodes[(new_inode_num / inodes_per_block) * inodes_per_block],
           inodes_per_block * sizeof(Inode));
...
void FileSystem::create(const std::string &filename) {
    ...
    update_inode_times(new_inode_num, true, true, true);
    for (int i = 0; i < 10; ++i)
        inodes[new_inode_num].direct_blocks[i] = 0;
...
void FileSystem::write(const std::string &filename, const std::string &data) {
    ...
    memcpy(inode_buffer, &inodes[(inode_num / inodes_per_block) * inodes_per_block],
           inodes_per_block * sizeof(Inode));
    journal->log_metadata_block(block_to_update, inode_buffer);
...
Inode FileSystem::get_inode(int inode_num) const {
    ...
    if (inode_num < 0 || inode_num >= NUM_INODES) {
        if (inode_num >= 0) { // Only log non-negative inode numbers
            qDebug() << "Warning: Inode number out of range:" << inode_num
                     << "(max:" << (NUM_INODES - 1) << ")";
        }
        return defaultInode;
    }
    ...
    if (!disk.is_open()) {
        if (inode_num != 0) { // Don't log for root inode
            qDebug() << "Warning: Attempting to get inode" << inode_num
                     << "from unmounted filesystem";
        }
        return defaultInode;
    }
    ...
    if (inode_num >= static_cast<int>(inodes.size())) {
        qDebug() << "Warning: Inode number" << inode_num << "exceeds inodes vector size"
                 << inodes.size();
        return defaultInode;
    }
...
void FileSystem::update_inode_times(int inode_num, bool access, bool modify, bool create) {
    ...
    if (create)
        inodes[inode_num].creation_time = now;
    if (access)
        inodes[inode_num].access_time = now;
    if (modify)
        inodes[inode_num].modification_time = now;
}
...
void FileSystem::fix_invalid_block_pointer(int inode_num, int block_index) {
    if (!is_valid_inode(inode_num)) {
        std::cerr << "Error: Cannot fix invalid block pointer for invalid inode " << inode_num
                  << std::endl;
        return;
    }
