...
QuotaManager::QuotaManager(FileSystem* fs) : fs(fs), grace_period(7 * 24 * 60 * 60) { // 7 days default
    // Initialize usage
    update_usage();
}
...
void QuotaManager::set_user_quota(int uid, int blocks_soft, int blocks_hard, int inodes_soft, int inodes_hard) {
    ...
    if ((blocks_used > blocks_soft && blocks_soft > 0) || (inodes_used > inodes_soft && inodes_soft > 0)) {
        quota.grace_period_start = time(nullptr);
    }
}

void QuotaManager::set_group_quota(int gid, int blocks_soft, int blocks_hard, int inodes_soft, int inodes_hard) {
...
bool QuotaManager::would_exceed_quota(int uid, int gid, int blocks_needed, int inodes_needed) {
    ...
    if (user_quota.blocks_hard_limit > 0 && user_quota.blocks_used + blocks_needed > user_quota.blocks_hard_limit) {
        return true;
    }
    
    if (user_quota.inodes_hard_limit > 0 && user_quota.inodes_used + inodes_needed > user_quota.inodes_hard_limit) {
        return true;
    }
    ...
    if (group_quota.blocks_hard_limit > 0 && group_quota.blocks_used + blocks_needed > group_quota.blocks_hard_limit) {
        return true;
    }
    
    if (group_quota.inodes_hard_limit > 0 && group_quota.inodes_used + inodes_needed > group_quota.inodes_hard_limit) {
        return true;
    }
...
void QuotaManager::update_usage() {
    ...
    for (auto& pair : user_quotas) {
        ...
        if (quota.blocks_soft_limit > 0 && quota.blocks_used > quota.blocks_soft_limit && quota.grace_period_start == 0) {
            quota.grace_period_start = now;
        }
        
        // Check inodes soft limit
        if (quota.inodes_soft_limit > 0 && quota.inodes_used > quota.inodes_soft_limit && quota.grace_period_start == 0) {
            quota.grace_period_start = now;
        }
    }
